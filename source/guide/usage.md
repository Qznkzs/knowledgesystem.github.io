...
- 日期应与文件名中的日期一致
- **`:category:` 必须写在这里**，不能只写在 YAML frontmatter 中（ABlog 从 `{post}` 指令读取分类信息）

## 分类与标签

### 分类（目录）

分类通过 `{post}` 指令的 `:category:` 选项声明。虽然建议文章放在对应分类目录下方便管理，但**实际的分类归属由 `:category:` 决定**，一篇文章只属于一个分类。

> **注意：** 不要在 YAML frontmatter 中设置 `category:`——ABlog 不会读取。必须写在 `{post}` 指令中。
